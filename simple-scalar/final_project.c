...
/* inst sequence type, used to order instructions in the ready list, if
   ...
   but execution will continue and compse correctly */
...
ruu_issue(void)
{
  ...
  for (n_issued=0;
	...
	if (prefetch_cache_enabled) {
		...
		// doesExist is true if the entry is found in the table. We have to go through the other checks if entry is found, else we insert
		if (doesExistIndex != -1) {
			// time to go through the cases..

			// first check if correct or incorrect
			...
			if (!correct && currentState == 0) {
				prev_addr_values[doesExistIndex] = (rs->addr - prev_addr);
				stride_values[doesExistIndex] = (rs->addr - prev_addr);
				state_values[doesExistIndex] = 1;
			} 
			// second case - moving to/being in steady state
			...
			// third case - steady state is over; back to initialization			
				...
				prev_addr_values[doesExistIndex] = (rs->addr);
				state_values[doesExistIndex] = 0;
			}
			// fourth case - detection of an irregular pattern			
			else if (!correct && currentState == 1) {
				prev_addr_values[doesExistIndex] = (rs->addr);
				stride_values[doesExistIndex] = (rs->addr - prev_addr);
				state_values[doesExistIndex] = 3;
			}
			// fifth case - no prediction state is over; back to transient		
			else if (correct && currentState == 3) {
				prev_addr_values[doesExistIndex] = (rs->addr);
				state_values[doesExistIndex] = 1;
			}
			// sixth case - irregular pattern	
			else if (!correct && currentState == 3) {
				prev_addr_values[doesExistIndex] = (rs->addr);
				stride_values[doesExistIndex] = (rs->addr - prev_addr);
			}
			...
			// entry not in table, we must insert, replace if necessary (the replacement scheme here is no good)
			tag_values[currentIndex] = rs->tag;
...
#include "machine.def"
  ...
  for (;;)
	...
	panic("LSQ_head/LSQ_tail wedged");
      ...
      /* service result compsions, also readies dependent operations */
