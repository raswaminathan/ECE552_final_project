...
static counter_t numCycles = 0;
static counter_t numCondBranches = 0;
static counter_t numRecentMemAccesses = 0;
static counter_t numStoreLoad = 0;
static counter_t numDataHazards = 0;
static int two_out[2];
static int one_out[2];
static int prevWasAStall = 0;
static long recentStores[2];
static int recentMemAccess[2];
...
stat_reg_counter(sdb, "sim_num_data_hazards",
		   "total number of data hazards",
		   &numDataHazards, numDataHazards, NULL);
stat_reg_counter(sdb, "sim_num_store_load",
		   "total number of store load hazards",
		   &numStoreLoad, numStoreLoad, NULL);
stat_reg_counter(sdb, "sim_num_recent_mem",
		   "total number of store/load same cycle",
		   &numRecentMemAccesses, numRecentMemAccesses, NULL);
stat_reg_counter(sdb, "sim_num_cond_branches",
		   "total number of cond branches",
		   &numCondBranches, numCondBranches, NULL);
...
#include "machine.def"
	...
	for (i = 0; i<3; i++) {
	  if (curInputs[i] == DNA) 
		continue;
	  if (two_out[1] == curInputs[i] || two_out[0] == curInputs[i] || one_out[1] == curInputs[i]
		|| one_out[0] == curInputs[i]) {
		
		// dont need to stall here if previous cycle already stalled due to hazard
		if (prevWasAStall == 0)
			numDataHazards++;
      ...
      if (verbose)
	{
      ...
      if (MD_OP_FLAGS(op) & F_MEM)
	{
	  if (recentMemAccess[0] == 1 || recentMemAccess[1] == 1) {
		numRecentMemAccesses++;
	  }
	  recentMemAccess[1] = recentMemAccess[0];
	  recentMemAccess[0] = 1;
	  sim_num_refs++;
	  if (MD_OP_FLAGS(op) & F_STORE) {
	    recentStores[1] = recentStores[0];
	    recentStores[0] = addr;
	    is_write = TRUE;
	  }

	  if (MD_OP_FLAGS(op) & F_LOAD) {
		if (addr == recentStores[1] || addr == recentStores[0]) {
			numStoreLoad++;	
		}
